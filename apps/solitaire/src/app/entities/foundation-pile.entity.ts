import { injectable } from 'inversify';
import { injectService } from '../injector/inject-service.function';
import { TriggerService } from '../solitaire-services/trigger.service';
import { WorldService } from '../core/world.service';
import { TriggerComponent } from '../solitaire-components/trigger.component';
import { PileComponent } from '../solitaire-components/pile.component';
import { FoundationComponent } from '../solitaire-components/foundation.component';

export type FoundationPileEntityArcheType = TriggerComponent &
  PileComponent &
  FoundationComponent;

@injectable()
export class FoundationPileEntity {
  private readonly triggerService = injectService(TriggerService);
  private readonly worldService = injectService(WorldService);

  public entity!: FoundationPileEntityArcheType;

  public constructor() {
    this.init();
  }

  private init() {
    this.initEntity();
  }

  private initEntity() {
    const { trigger } = this.createTriggerComponent();
    const { pile } = this.createPileComponent();
    const { foundation } = this.createFoundationComponent();

    const entity = this.worldService
      .getWorld<FoundationPileEntityArcheType>()
      .add({
        trigger,
        pile,
        foundation,
      });
    this.entity = entity;
  }

  private createTriggerComponent() {
    const tableauPileInstance =
      this.triggerService.createTriggerInstance('foundation_pile');
    return new TriggerComponent(tableauPileInstance);
  }

  private createPileComponent() {
    return new PileComponent([]);
  }

  private createFoundationComponent() {
    return new FoundationComponent();
  }
}
